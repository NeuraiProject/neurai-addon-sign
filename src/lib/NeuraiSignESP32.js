var NeuraiSignESP32Bundle = (function () {
    ...
    class SerialProtocol {
        ...
        async writeCommand(command, timeoutMs) {
     ...
     */
    class SerialConnection extends SerialProtocol {
        constructor(options) {
    ...
    */
    ...
    function toBech32(data, version, prefix) {
      ...
      return version === 0
        ? distExports$1.bech32.encode(prefix, words)
        : distExports$1.bech32m.encode(prefix, words);
    }
    /**
    ...
    function pqAddressFromPublicKey(pqPubkey, params, witnessScriptHex = DEFAULT_WITNESS_SCRIPT_HEX) {
        ...
        const words = [params.witnessVersion, ...distExports$1.bech32m.toWords(commitment)];
        return distExports$1.bech32m.encode(params.hrp, words);
    }
    ...
    function decodeAddress(address) {
        ...
        if (isPQAddress(normalized)) {
            const decoded = distExports$1.bech32m.decode(normalized);
            const witnessVersion = decoded.words[0];
            const commitment = bufferExports.Buffer.from(distExports$1.bech32m.fromWords(decoded.words.slice(1)));
    ...
    class NeuraiESP32 {
         ...
         */
        async getInfo() {
            const response = await this.serial.sendCommand({ action: "info" }, 35000);
            this.assertSuccess(response);
         ...
         */
        async depinSessionBegin(token, options) {
            const command = {
                action: "depin_session_begin",
                token,
            };
            if (options?.ttlMinutes !== undefined)
                command.ttl_minutes = options.ttlMinutes;
            if (options?.ratePerMin !== undefined)
                command.rate_per_min = options.ratePerMin;
         ...
         */
        async getDepinIdentity() {
            const response = await this.serial.sendCommand({ action: "get_depin_identity", ...this.depinSessionFields() }, 10000);
            this.assertSuccess(response);
            return response;
        }
        /**
         ...
         */
        async depinDecrypt(depinMessageHex) {
            const response = await this.serial.sendCommand({
                action: "depin_decrypt",
                ...(await this.depinDecryptWirePayload("depin_message", "depin_message_b64", depinMessageHex)),
                ...this.depinSessionFields(),
            }, 35000);
            this.assertSuccess(response);
            return response;
         ...
         */
        async depinDecryptPayload(encryptedPayloadHex) {
            const response = await this.serial.sendCommand({
                action: "depin_decrypt_payload",
                ...(await this.depinDecryptWirePayload("encrypted_payload", "encrypted_payload_b64", encryptedPayloadHex)),
                ...this.depinSessionFields(),
            }, 35000);
            this.assertSuccess(response);
            return response;
         ...
         */
        async depinDecryptWirePayload(hexField, base64Field, hexPayload) {
            const capability = await this.getDepinBulkCapability();
            const maxBytes = capability?.depin_max_decrypt_bytes;
            const supportsBase64 = capability?.capabilities?.includes("depin_bulk_decrypt_b64") === true &&
            ...
            const bytes = bufferExports.Buffer.from(hexPayload, "hex");
            if (supportsBase64) {
                if (bytes.length > maxBytes) {
                    throw new RangeError(`DePIN decrypt payload is ${bytes.length} bytes; this device accepts at most ${maxBytes} bytes. Paginate the response before decrypting it.`);
                }
            ...
            if (bytes.length > legacyMaxBytes) {
                throw new RangeError(`DePIN decrypt payload is ${bytes.length} bytes and exceeds the ${legacyMaxBytes}-byte legacy hex transport limit. Update the firmware or paginate the response.`);
            }
            return { [hexField]: hexPayload };
        }
        /**
         ...
         */
        async depinSignDigest(digestHex) {
            const response = await this.serial.sendCommand({ action: "depin_sign_digest", digest: digestHex }, 35000);
            this.assertSuccess(response);
            return response;
        }
        /**
         ...
         */
        async depinSessionEnd() {
            const response = await this.serial.sendCommand({ action: "depin_session_end", ...this.depinSessionFields() }, 10000);
            this.assertSuccess(response);
            this.depinSessionKey = null;
        }
        /**
         ...
         */
        async depinSessionStatus() {
            const response = await this.serial.sendCommand({ action: "depin_session_status", ...this.depinSessionFields() }, 10000);
            this.assertSuccess(response);
    ...
    var ecc = /*@__PURE__*/getDefaultExportFromCjs(distExports);
     ...
     */
    class DepinDeviceIdentityError extends Error {
    ...
    // ---------------------------------------------------------------------------
    function serializeVarString(value) {
        const bytes = bufferExports.Buffer.from(value, "utf8");
        ...
        return bufferExports.Buffer.concat([bufferExports.Buffer.from(encode$e(bytes.length).buffer), bytes]);
    }
    /**
     ...
     */
    function neuraiSignedMessageDigest(message) {
        return bufferExports.Buffer.from(hash256$1(bufferExports.Buffer.concat([serializeVarString(NEURAI_MESSAGE_MAGIC), serializeVarString(message)])));
    }
    function derToCompactLowS(derHex) {
        if (typeof derHex !== "string" ||
            derHex.length === 0 ||
            derHex.length % 2 !== 0 ||
            !/^[0-9a-f]+$/.test(derHex)) {
            throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "The device returned a signature that is not canonical lowercase DER hex");
        }
        let signature$1;
        try {
            signature$1 = bufferExports.Buffer.from(signature.decode(bufferExports.Buffer.concat([bufferExports.Buffer.from(derHex, "hex"), bufferExports.Buffer.from([0x01])])).signature);
        }
        catch (error) {
            throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "The device returned a signature that is not canonical DER", error);
        }
        if (signature$1.length !== 64) {
            throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", `Compact signature must be 64 bytes; got ${signature$1.length}`);
        }
        const s = BigInt(`0x${signature$1.subarray(32).toString("hex")}`);
        if (s === 0n || s >= SECP256K1_ORDER) {
            throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "The device returned a signature with an invalid s scalar");
        }
        if (s > SECP256K1_HALF_ORDER) {
            const normalized = bufferExports.Buffer.from((SECP256K1_ORDER - s).toString(16).padStart(64, "0"), "hex");
            normalized.copy(signature$1, 32);
        }
        return signature$1;
    }
    function compactToDer(compact) {
        const encoded = bufferExports.Buffer.from(signature.encode(compact, 0x01));
        return encoded.subarray(0, encoded.length - 1);
    }
    function assertOperationCount(value) {
        if (!Number.isSafeInteger(value) || value < 0) {
            throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "The device returned an invalid operation counter");
        }
    }
    function decodeCanonicalBase64(value) {
        if (value.length % 4 !== 0 ||
            !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(value)) {
            throw new DepinDeviceIdentityError("DECRYPT_AUTH_FAILED", "The device returned malformed Base64 plaintext");
        }
        const decoded = bufferExports.Buffer.from(value, "base64");
        if (decoded.toString("base64") !== value) {
            throw new DepinDeviceIdentityError("DECRYPT_AUTH_FAILED", "The device returned non-canonical Base64 plaintext");
        }
        return decoded;
    }
    /**
     ...
     */
    function derSignatureToRecoverable(derHex, digest, expectedPubkeyHex) {
        ...
        for (let recoveryId = 0; recoveryId < 4; recoveryId += 1) {
            let recovered = null;
            try {
                recovered = ecc.recover(digest, compact, recoveryId, true);
            }
            catch {
                continue;
            }
            if (recovered && bufferExports.Buffer.from(recovered).toString("hex") === expected) {
                const header = 27 + recoveryId + 4; // compressed key
                return bufferExports.Buffer.concat([bufferExports.Buffer.from([header]), compact]).toString("base64");
            }
        }
        throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "Could not recover the device's public key from its signature: it signed a different digest or with a different key");
    }
    // ---------------------------------------------------------------------------
    // Permissions
    // ---------------------------------------------------------------------------
    function canonicalizePermissions(permissions, context) {
        const list = permissions ?? ["receive"];
        if (!Array.isArray(list) || list.length === 0) {
            throw new DepinDeviceIdentityError("SESSION_PERMISSION_MISMATCH", `${context}: permissions must be a non-empty array`);
        }
        const seen = new Set();
        for (const permission of list) {
            if (!PERMISSION_ORDER.includes(permission)) {
                throw new DepinDeviceIdentityError("SESSION_PERMISSION_MISMATCH", `${context}: unknown permission ${JSON.stringify(permission)}`);
            }
            if (seen.has(permission)) {
                throw new DepinDeviceIdentityError("SESSION_PERMISSION_MISMATCH", `${context}: duplicate permission ${permission}`);
            }
            seen.add(permission);
        }
    ...
    function requiredPermission(command) {
    ...
    function authPreimageFor(command) {
        switch (command.operation) {
            case "request":
    ...
    function messageDigestFor(context) {
        if (!Number.isSafeInteger(context.timestamp) ||
            context.timestamp < 0 ||
            ...
            throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "The message signing context contains non-canonical fields");
        }
        const timestamp = bufferExports.Buffer.alloc(8);
        timestamp.writeBigInt64LE(BigInt(context.timestamp));
        const payload = bufferExports.Buffer.from(context.encryptedPayloadHex, "hex");
        return bufferExports.Buffer.from(hash256$1(bufferExports.Buffer.concat([
            serializeVarString(context.token),
            serializeVarString(context.senderAddress),
            timestamp,
            bufferExports.Buffer.from([context.messageType]),
            bufferExports.Buffer.from(encode$e(payload.length).buffer),
            payload,
        ])));
    }
    // ---------------------------------------------------------------------------
     ...
     */
    async function createDepinDeviceIdentity(device, options) {
        ...
        if (options.expectedNetwork !== "mainnet" && options.expectedNetwork !== "test") {
            throw new DepinDeviceIdentityError("IDENTITY_NETWORK_MISMATCH", "options.expectedNetwork must be 'mainnet' or 'test'");
        }
        // Frozen copy: later decisions never read a caller-mutable array.
        const expectedPermissions = Object.freeze(canonicalizePermissions(options.sessionPermissions, "options.sessionPermissions"));
        const autoSession = options.autoSession !== false;
        let identity = null;
        let maxDecryptBytes = null;
        let ensureInFlight = null;
        let requestedMinimumSeconds = 0;
        async function negotiate() {
            const ping = await device.ping();
            if (ping.protocol_version !== 2) {
                throw new DepinDeviceIdentityError("UNSUPPORTED_PROTOCOL", `This adapter requires DePIN protocol 2; the device announces ${JSON.stringify(ping.protocol_version)}`);
            }
            const capabilities = ping.capabilities ?? [];
            for (const capability of REQUIRED_CAPABILITIES) {
                if (!capabilities.includes(capability)) {
                    throw new DepinDeviceIdentityError("MISSING_CAPABILITY", `Firmware does not advertise the ${capability} capability; refusing to downgrade`);
                }
            }
            const limit = ping.depin_max_decrypt_bytes;
            maxDecryptBytes = Number.isSafeInteger(limit) && limit > 0 ? limit : null;
        }
        async function validateIdentity() {
            const announced = await device.getDepinIdentity();
            if (announced.protocol_version !== 2) {
                throw new DepinDeviceIdentityError("UNSUPPORTED_PROTOCOL", `The DePIN identity announces protocol ${JSON.stringify(announced.protocol_version)}, expected 2`);
            }
            let network;
            if (announced.network === "xna") {
                ...
                throw new DepinDeviceIdentityError("IDENTITY_NETWORK_MISMATCH", `Device identity announced an unknown network ${JSON.stringify(announced.network)}`);
            }
            if (network !== options.expectedNetwork) {
                throw new DepinDeviceIdentityError("IDENTITY_NETWORK_MISMATCH", `Device identity is on ${network}, the application expects ${options.expectedNetwork}`);
            }
            const expectedPath = DEPIN_IDENTITY_PATHS[options.expectedNetwork];
            if (announced.path !== expectedPath) {
                throw new DepinDeviceIdentityError("IDENTITY_PATH_MISMATCH", `Device identity uses ${JSON.stringify(announced.path)}, expected ${expectedPath}`);
            }
            if (typeof announced.pubkey !== "string" || announced.pubkey.length !== 66) {
                throw new DepinDeviceIdentityError("IDENTITY_KEY_MISMATCH", "Device DePIN pubkey must be a 33-byte compressed SEC1 key");
            }
            const pubkey = bufferExports.Buffer.from(announced.pubkey, "hex");
            if (!ecc.isPoint(pubkey) || !ecc.isPointCompressed(pubkey)) {
                throw new DepinDeviceIdentityError("IDENTITY_KEY_MISMATCH", "Device DePIN pubkey is not a valid compressed secp256k1 point");
            }
            ...
            if (derived !== announced.address) {
                throw new DepinDeviceIdentityError("IDENTITY_KEY_MISMATCH", "Device DePIN address does not match its announced public key");
            }
            return announced;
        }
        function assertSessionUsable(status) {
            if (!status.active) {
                throw new DepinDeviceIdentityError("SESSION_EXPIRED", "The DePIN session is no longer active");
            }
            if (status.token !== options.token) {
                throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", `The active session is bound to ${JSON.stringify(status.token)}, not ${JSON.stringify(options.token)}`);
            }
            if (!Array.isArray(status.permissions)) {
                throw new DepinDeviceIdentityError("SESSION_PERMISSION_MISMATCH", "Firmware advertises depin_session_permissions but the status omits them");
            }
            if (!Number.isSafeInteger(status.expires_in_s) || status.expires_in_s < 0) {
                throw new DepinDeviceIdentityError("SESSION_EXPIRED", "An active DePIN session must report a non-negative integer expires_in_s");
            }
        ...
        async function rejectOpenedSession(error) {
            identity = null;
            if (device.getDepinSessionKey()) {
                try {
                    ...
                    throw new DepinDeviceIdentityError("DEVICE_DISCONNECTED", "The new DePIN session was invalid and could not be revoked; its capability key was retained", { validationError: error, closeError });
                }
            }
            throw error;
        }
        async function ensureOnce(minRemaining) {
            await negotiate();
            ...
            if (device.getDepinSessionKey()) {
                ...
                try {
                    ...
                    identity = null;
                    throw new DepinDeviceIdentityError("DEVICE_DISCONNECTED", "Could not read the DePIN session status from the device", error);
                }
                if (status.active) {
                    const granted = assertSessionUsableSafely(status);
                    const budgetOk = typeof status.expires_in_s === "number" && status.expires_in_s >= minRemaining;
                    if (granted && samePermissions(granted, expectedPermissions) && budgetOk) {
                        ...
                        if (!autoSession) {
                            ...
                            throw new DepinDeviceIdentityError("SESSION_TIME_BUDGET_INSUFFICIENT", `The active session has ${status.expires_in_s}s left, below the ${minRemaining}s budget`);
                        }
                        await closeSession();
                    }
                }
            ...
            if (!autoSession) {
                throw new DepinDeviceIdentityError("SESSION_REQUIRED", "No active DePIN session and autoSession is disabled; open one explicitly");
            }
            let opened;
            try {
                ...
                throw new DepinDeviceIdentityError("SESSION_DECLINED", "The device did not approve the DePIN session", error);
            }
            try {
                if (opened.token !== options.token) {
                    throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", "The device opened a session for a different token");
                }
                if (!device.getDepinSessionKey()) {
                    throw new DepinDeviceIdentityError("MISSING_CAPABILITY", "The device did not return a session capability key");
                }
                ...
                if (!samePermissions(granted, expectedPermissions)) {
                    throw new DepinDeviceIdentityError("SESSION_PERMISSION_MISMATCH", `Requested [${expectedPermissions.join(", ")}] but the device granted [${granted.join(", ")}]`);
                }
                if (status.expires_in_s < minRemaining) {
                    throw new DepinDeviceIdentityError("SESSION_TIME_BUDGET_INSUFFICIENT", `A new session only offers ${status.expires_in_s}s, below the ${minRemaining}s budget; raise ttlMinutes or lower the budget`);
                }
                identity = await validateIdentity();
            }
            catch (error) {
        ...
        async function ensure(ensureOptions) {
            const minRemaining = ensureOptions?.minRemainingSeconds ?? 0;
            if ((!Number.isInteger(minRemaining) || minRemaining < 0)) {
                throw new DepinDeviceIdentityError("SESSION_TIME_BUDGET_INSUFFICIENT", "minRemainingSeconds must be a non-negative integer");
            }
        ...
        function assertSessionUsableSafely(status) {
            try {
        ...
        function assertPermission(command) {
            const needed = requiredPermission(command);
            if (!expectedPermissions.includes(needed)) {
                throw new DepinDeviceIdentityError("SESSION_PERMISSION_INSUFFICIENT", `This operation needs the ${needed} permission; the session holds [${expectedPermissions.join(", ")}]`);
            }
        }
        function requireIdentity() {
            if (!identity) {
                throw new DepinDeviceIdentityError("SESSION_REQUIRED", "Call session.ensure() before using the identity");
            }
            return identity;
        }
        function authCommandFor(context) {
            const address = requireIdentity().address;
            switch (context.kind) {
                case "request":
                    if (context.token !== options.token) {
                        throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", "The signing context token does not equal the session token");
                    }
                    if (context.address !== address) {
                        throw new DepinDeviceIdentityError("NOT_FOR_THIS_IDENTITY", "The signing context names a different address than the device identity");
                    }
                    return {
                        operation: "request",
                        type: context.type,
                ...
                case "clear":
                    if (context.scope !== options.token) {
                        throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", "The signing context scope does not equal the session token");
                    }
                    if (context.address !== address) {
                        throw new DepinDeviceIdentityError("NOT_FOR_THIS_IDENTITY", "The signing context names a different address than the device identity");
                    }
                ...
                default: {
                    ...
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", `Unknown signing context ${JSON.stringify(exhaustive)}`);
                }
            }
        }
        const adapter = {
            ...
            async signMessage(preimage, context) {
                if (!context) {
                    ...
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "A hardware identity needs the structured signing context (depin-msg >= 3.1.0)");
                }
                const preliminaryCommand = context.kind === "request"
                    ? {
                        operation: "request",
                        type: context.type,
                        token: context.token,
                ...
                const contextScope = preliminaryCommand.operation === "clear"
                    ? preliminaryCommand.scope
                    : preliminaryCommand.token;
                if (contextScope !== options.token) {
                    throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", "The signing context token or scope does not equal the session token");
                }
                if (authPreimageFor(preliminaryCommand) !== preimage) {
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "The structured signing context does not rebuild the caller's authentication preimage");
                }
                await ensure();
                ...
                if (typeof response.pubkey !== "string" ||
                    !/^(?:02|03)[0-9a-f]{64}$/.test(response.pubkey) ||
                    response.pubkey !== declared) {
                    throw new DepinDeviceIdentityError("IDENTITY_KEY_MISMATCH", "The device signed with a key other than its announced DePIN identity");
                }
            ...
            async signDigest(digest, context) {
                if (!context) {
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "Publishing needs the structured message context (depin-msg >= 3.1.0)");
                }
                assertPermission({ operation: "publish" });
                if (context.kind !== "message" || context.token !== options.token) {
                    throw new DepinDeviceIdentityError("SESSION_TOKEN_MISMATCH", "The message context token does not equal the session token");
                }
                if (!(digest instanceof Uint8Array) || digest.length !== 32) {
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "The message digest must be exactly 32 bytes");
                }
                const rebuiltDigest = messageDigestFor(context);
                if (!rebuiltDigest.equals(bufferExports.Buffer.from(digest))) {
                    throw new DepinDeviceIdentityError("SIGNING_CONTEXT_REQUIRED", "The structured message context does not rebuild the caller's digest");
                }
                await ensure();
                const address = requireIdentity().address;
                if (context.senderAddress !== address) {
                    throw new DepinDeviceIdentityError("NOT_FOR_THIS_IDENTITY", "The message context names a different sender than the device identity");
                }
                let response;
                try {
                ...
                assertOperationCount(response.op_count);
                const pubkey = bufferExports.Buffer.from(requireIdentity().pubkey, "hex");
                if (!ecc.verify(bufferExports.Buffer.from(digest), pubkey, compact)) {
                    throw new DepinDeviceIdentityError("INVALID_DEVICE_SIGNATURE", "The device's message signature does not verify against the digest built by the host");
                }
            ...
            async openReply(encryptedHex) {
                if (typeof encryptedHex !== "string" || !/^[0-9a-f]*$/.test(encryptedHex) ||
                    ...
                    throw new DepinDeviceIdentityError("DECRYPT_AUTH_FAILED", "The envelope to decrypt must be non-empty canonical hex");
                }
                if (maxDecryptBytes !== null && encryptedHex.length / 2 > maxDecryptBytes) {
                    throw new DepinDeviceIdentityError("PAYLOAD_TOO_LARGE", `The envelope is ${encryptedHex.length / 2} bytes; this device accepts at most ${maxDecryptBytes}. Lower the page size and request a fresh challenge.`);
                }
                await ensure();
                requireIdentity();
                let response;
                try {
                    ...
                    identity = null;
                    if (/not[_ ]for|not encrypted|recipient/i.test(String(error?.message ?? error))) {
                        throw new DepinDeviceIdentityError("NOT_FOR_THIS_IDENTITY", "The envelope is not addressed to this device identity", error);
                    }
                    throw new DepinDeviceIdentityError("DECRYPT_AUTH_FAILED", "The device could not open the envelope for this identity", error);
                }
                if (typeof response.plaintext_b64 !== "string") {
                    throw new DepinDeviceIdentityError("DECRYPT_AUTH_FAILED", "The device returned no plaintext for the envelope");
                }
                assertOperationCount(response.op_count);
